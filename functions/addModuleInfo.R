...
addModuleInfo = function(dataSet){
  keyInfo_attempt = read.delim("mappings/OLI_start_attempt_week_mapping.txt", header=FALSE, stringsAsFactors=FALSE)
  names(keyInfo_attempt) = c("info","module")
  
  keyInfo_session = read.delim("mappings/OLI_start_session_week_mapping.txt", header=FALSE, stringsAsFactors=FALSE)
  names(keyInfo_session) = c("info","module")
  
  keyInfo_activities = join(keyInfo_attempt,keyInfo_session)
  
  keyInfo_page = read.delim("mappings/OLI_view_page_week_mapping.txt", header=FALSE, stringsAsFactors=FALSE)
